...
!SLIDE small
# Validations
Les validations ont lieu au moment de la persistence.  
Au terminal: rails c --sandbox  

    @@@ Ruby
    Task.new.new_record?  # not persisted 
    Task.save
    Task.new.new_record?  # persisted
...
### Le test unitaire (a posteriori)
    ...
    test "title should be present"  do
      ...
      assertion_message = %Q[errors[:title] does not contain "#{expected_error}"]

      assert t.errors[:title].include?(expected_error), assertion_message
    end
...
!SLIDE bullets small
## Tester la validation dans le browser corrections
...
- Rajouter une validation sur la présence de "title" sur le model Project  
- Rajouter une validation sur le "due date" de Project qui vérifie que le nouveau projet est dans le futur.  
